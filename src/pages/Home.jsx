...
const flattenFiles = (nodes, pathArray = []) => {
  ...
  for (const node of nodes) {
    if (node.type === "file") {
      ...
      files = files.concat(
        flattenFiles(node.children, [...pathArray, node.name]),
      );
    }
  }
  return files;
    ...
    <div className="w-full bg-white font-sans text-gray-800">
      ...
      <section className="relative pt-20 pb-24 px-6 text-center overflow-hidden flex flex-col items-center justify-center">
        <div className="absolute top-10 text-xs font-semibold text-blue-600 bg-blue-50 px-3 py-1 rounded-full mb-8">
          ✦ UIT Knowledge Hub - Không gian học tập tập trung
        </div>

        <h1 className="text-4xl md:text-6xl font-bold mb-4 mt-8 leading-tight tracking-tight">
          Tìm kiếm & Chia sẻ
          <br />
          <span className="text-blue-600">Tài nguyên Học tập UIT</span>
        </h1>

        <p className="text-base md:text-lg mb-10 text-gray-500 max-w-2xl mx-auto">
          Hệ thống chia sẻ tài nguyên học tập, đề thi, slide bài giảng, đồ án
          mẫu và diễn đàn trao đổi kinh nghiệm dành riêng cho sinh viên Đại học
        ...
        <form
          ...
          className="w-full max-w-3xl relative shadow-lg rounded-full flex bg-white p-2 border border-gray-100"
        >
          ...
          <button
            type="submit"
            className="bg-blue-700 hover:bg-blue-800 text-white px-8 py-3 rounded-full font-medium transition"
          >
            Tìm kiếm
          </button>
        </form>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3 text-sm text-gray-500">
          <span>Xu hướng:</span>
          <span
            onClick={() => handleTrendingClick("CTRR")}
            className="px-4 py-1.5 border border-gray-200 rounded-full cursor-pointer hover:bg-gray-50 transition"
          >
            Cấu trúc rời rạc
          </span>
          <span
            onClick={() => handleTrendingClick("DSA")}
            className="px-4 py-1.5 border border-gray-200 rounded-full cursor-pointer hover:bg-gray-50 transition"
          >
            Cấu trúc dữ liệu
          </span>
          <span
            onClick={() => handleTrendingClick("OOP")}
            className="px-4 py-1.5 border border-gray-200 rounded-full cursor-pointer hover:bg-gray-50 transition"
          >
            Lập trình HĐT
          </span>
          <span
            onClick={() => handleTrendingClick("Nhập Môn Mạng")}
            className="px-4 py-1.5 border border-gray-200 rounded-full cursor-pointer hover:bg-gray-50 transition"
          >
            Mạng máy tính
          </span>
          ...
          <div className="flex justify-between items-end mb-10">
            <div>
              ...
              <p className="text-gray-500">
                Cập nhật những tài liệu vừa được cộng đồng chia sẻ
              </p>
            </div>
            <Link
              to="/documents"
              className="text-blue-600 font-medium hover:text-blue-800 transition flex items-center gap-1"
            >
